...
#define EXAMPLE_NUM_TABLE_ENTRIES (6)
 ...
 */
uint32_t CalcActionId[EXAMPLE_NUM_TABLE_ENTRIES] = {3,1,5,4,2,0};
...
int main(void)
{
    ...
    printf("Get ActionId\n\r");
    Result = XilVitisNetP4TableGetActionId(CalcTableCtxPtr, "operation_xor", &ActionId);
    if (Result != XIL_VITIS_NET_P4_SUCCESS)
    {
        DISPLAY_ERROR(Result);
        goto target_exit;
    }
    printf("Action ID for operation_xor is %d\n\r", ActionId);

    char ActionName[100] = "\0";
    uint32_t actionNumberSize = 1000;
    Result = XilVitisNetP4TableGetActionNameById(CalcTableCtxPtr, (uint32_t)(1), ActionName, actionNumberSize);
    if (Result != XIL_VITIS_NET_P4_SUCCESS)
    {
        DISPLAY_ERROR(Result);
        goto target_exit;
    }
    printf("The name for ActionID = 1 is %s\n\r", ActionName);

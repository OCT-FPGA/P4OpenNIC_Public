...
module p2p_advCalc_p4_250mhz #(
  ...
  generate for (genvar i = 0; i < NUM_INTF; i++) begin
   ...
   calc_divide_ip calc_divide_ip_inst (
      ...
      .m_axis_dout_tdata      ({user_extern_in.calc_divide.quotient,user_extern_in.calc_divide.remainder})
   );

   calc_square_root_ip calc_square_root_ip_inst (
      .aclk                    (axis_aclk),
      .s_axis_cartesian_tvalid (user_extern_out_valid.calc_square_root),
      .s_axis_cartesian_tdata  (user_extern_out.calc_square_root),
      .m_axis_dout_tvalid      (user_extern_in_valid.calc_square_root),
      .m_axis_dout_tdata       (user_extern_in.calc_square_root)
   );   
	// -------------------------------------------------------------------------
    end else begin      
...
endmodule: p2p_advCalc_p4_250mhz
